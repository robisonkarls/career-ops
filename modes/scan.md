...
6. **Filter by title** using `title_filter` from `portals.yml`:
   ...
   - `seniority_boost` keywords prioritize but are not required
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   WebSearch results may be stale (Google caches results for weeks or months). To avoid evaluating expired offers, verify each new Level 3 URL with Playwright. Levels 1 and 2 are inherently real-time and don't require this check.
   ...
   **Don't abort the entire scan if one URL fails.** If `browser_navigate` returns an error (timeout, 403, etc.), mark as `skipped_expired` and continue.
...
The `careers_url` should point to the company's own careers page whenever available. Many companies use Workday, Greenhouse, or Lever under the hood but expose job IDs only through their corporate domain. Using the ATS URL directly when a corporate page exists can cause false 410 errors because job IDs don't match.
